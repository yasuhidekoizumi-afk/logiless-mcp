...
export interface LogilessError {
  code?: number;
  message?: string;
...
export interface ActualInventorySummary {
  id: number;
  layer: string;
  deadline?: string;
  lot_number?: string;
...
export interface LogicalInventorySummary {
  id: number;
  layer: string;
  ordered: number;
  in_transit: number;
  ...
  shipped: number;
  issued: number;
  is_reorder_level: number;
  reached_reorder_level_at?: string;
  created_at: string;
  updated_at: string;
  article: Partial<Article>;
  warehouse: Partial<Warehouse>;
...
export interface SalesOrder {
  ...
  wrapping?: string;
  tags?: string[];
  purchasing_times?: number;
  ordered_at?: string;
  created_at: string;
  updated_at: string;
  lines: SalesOrderLine[];
  outbound_deliveries?: OutboundDelivery[];
  store: Partial<Store>;
}
...
export interface OutboundDelivery {
  ...
  recipient_address1: string;
  delivery_method: string;
  delivery_tracking_numbers: string[];
  scheduled_shipping_date?: string;
  ...
  warehouse?: number;
  store?: number;
  sales_order?: { id: string; code: string };
}
